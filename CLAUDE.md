...
**DEPLOYED (2026-07-13):** GitHub `ShehrozHanif/aftercare` (public, auto-deploys both platforms on push). Frontend: **https://aftercare-theta.vercel.app** (Vercel project `aftercare`, root dir `frontend`, `frontend/vercel.json` pins the nextjs preset — the auto-created project defaulted to "Other" and 404'd; `NEXT_PUBLIC_API_URL` set in production env). Backend: **https://aftercare-backend.onrender.com** (Render web service `srv-d99vf8naqgkc738t8jkg`, Singapore, free tier, root dir `backend`, build `uv sync`, start `uv run uvicorn app.main:app --host 0.0.0.0 --port $PORT`; Python 3.14 works on Render). DB: Render Postgres `aftercare-db` (**free tier expires 2026-08-11**), `DATABASE_URL` uses the internal URL with the prefix rewritten to `postgresql+asyncpg://`. All env vars from `backend/.env` are set on Render, so WhatsApp + LLM work from the deployed backend. Verified live: escalation E2E (LLM WARNING + red dashboard row), then acked + resolved back to green. Render free tier sleeps after ~15 min idle (~30–60s cold start) — open the app a few minutes before demoing.

**▶ RESUME HERE — remaining work, in order:**
1. **Twilio webhook → permanent URL**: point the sandbox webhook ("When a message comes in") at `https://aftercare-backend.onrender.com/whatsapp/incoming` (POST) — replaces the flaky localtunnel setup. Then test the WhatsApp loop from the user's phone.
2. Before the demo: reset the **deployed** DB state (ack + `POST /patients/1/resolve`, or drop/recreate the Render DB) and locally delete `backend/aftercare.db` + restart; run the §13 script once as rehearsal. `jounry.md` is the original design journal; it contains the full rationale behind the safety rules and the no-vitals / no-training decisions — read it if a design choice here seems arbitrary.
3. **After the hackathon: rotate the OpenAI key, Twilio auth token, and Render API key** (all passed through chat).
